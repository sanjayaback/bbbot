...
async def hybrid_retrieve(
    ...
    params = {
        ...
        "q": question,
        "w": workspace_id,
        "has_sources": bool(kb_ids or doc_ids),
        "use_kb": bool(kb_ids),
        "kb": kb_ids or None,
        "use_doc": bool(doc_ids),
        "docs": doc_ids or None,
        "candidate_limit": max(limit * 4, 24),
    }
    rows = (await db.execute(text(sql), params)).mappings().all()
    reranked = heuristic_rerank(question, [dict(r) for r in rows], limit=max(limit * 2, limit))
    relevant = [item for item in reranked if float(item.get("rerank_score", 0)) >= settings.retrieval_min_score]
