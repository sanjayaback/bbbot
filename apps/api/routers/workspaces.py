...
from apps.api.schemas import WorkspaceCreate, WorkspaceUpdate, KnowledgeBaseCreate, KnowledgeBaseUpdate, MemberUpsert, MemberByEmail
...
async def upsert_member(workspace_id: str, body: MemberUpsert, request: Request,p:Principal=Depends(current_principal),db:AsyncSession=Depends(get_db)):
    ...
    if existing_role == "owner" and body.role != "owner":
