...
async def create_session(body: ChatSessionCreate, request: Request, principal: Principal = Depends(current_principal), db: AsyncSession = Depends(get_db)):
    ...
    if body.knowledge_base_ids:
        count = (
            await db.execute(
                text("SELECT count(*) FROM knowledge_bases WHERE workspace_id=:w AND id=ANY(CAST(:ids AS uuid[]))"),
                {"w": body.workspace_id, "ids": body.knowledge_base_ids},
            )
        ).scalar_one()
        if count != len(set(body.knowledge_base_ids)):
            raise HTTPException(400, "One or more knowledge bases are invalid")
    if body.document_ids:
        count = (
            await db.execute(
                text("SELECT count(*) FROM documents WHERE workspace_id=:w AND archived_at IS NULL AND id=ANY(CAST(:ids AS uuid[]))"),
                {"w": body.workspace_id, "ids": body.document_ids},
            )
        ).scalar_one()
        if count != len(set(body.document_ids)):
            raise HTTPException(400, "One or more documents are invalid")

    session_id = str(uuid.uuid4())
    await db.execute(
        text("INSERT INTO chat_sessions(id,workspace_id,user_id,title) VALUES(:id,:w,:u,:t)"),
        {"id": session_id, "w": body.workspace_id, "u": principal.user_id, "t": body.title or "New chat"},
    )
    for source_type, ids in (("knowledge_base", body.knowledge_base_ids), ("document", body.document_ids)):
        for source_id in ids:
            await db.execute(
                text("INSERT INTO chat_session_sources(id,session_id,source_type,source_id) VALUES(:id,:s,:t,:x)"),
                {"id": str(uuid.uuid4()), "s": session_id, "t": source_type, "x": source_id},
            )
    await write_audit(
        db,
...
async def session_detail(session_id: str, principal: Principal = Depends(current_principal), db: AsyncSession = Depends(get_db)):
    session = (
        await db.execute(
            text("SELECT id,workspace_id,title,created_at FROM chat_sessions WHERE id=:s AND user_id=:u"),
            {"s": session_id, "u": principal.user_id},
        )
    ).mappings().first()
    if not session:
    ...
    sources = (
        await db.execute(text("SELECT source_type,source_id FROM chat_session_sources WHERE session_id=:s"), {"s": session_id})
    ).mappings().all()
...
async def delete_session(session_id: str, principal: Principal = Depends(current_principal), db: AsyncSession = Depends(get_db)):
    row = (
        await db.execute(
            text("SELECT workspace_id FROM chat_sessions WHERE id=:s AND user_id=:u"),
            {"s": session_id, "u": principal.user_id},
        )
    ).mappings().first()
    if not row:
        raise HTTPException(404, "Session not found")
    await require_workspace_role(str(row["workspace_id"]), principal, db, {"owner", "admin", "editor", "viewer"})
...
async def ask(session_id: str, body: AskRequest, request: Request, principal: Principal = Depends(current_principal), db: AsyncSession = Depends(get_db)):
    session = (
        await db.execute(
            text("SELECT id,workspace_id FROM chat_sessions WHERE id=:s AND user_id=:u"),
            {"s": session_id, "u": principal.user_id},
        )
    ).mappings().first()
    if not session:
    ...
    await db.execute(
        text("INSERT INTO messages(id,session_id,user_id,role,content,status) VALUES(:id,:s,:u,'user',:c,'completed')"),
        {"id": user_message_id, "s": session_id, "u": principal.user_id, "c": body.question},
    )
    assistant_message_id = str(uuid.uuid4())
    await db.execute(
        text("INSERT INTO messages(id,session_id,user_id,role,content,status) VALUES(:id,:s,:u,'assistant','','generating')"),
        {"id": assistant_message_id, "s": session_id, "u": principal.user_id},
    )
    llm = create_llm(api_key)
    usage_id = str(uuid.uuid4())
    await db.execute(
        text("INSERT INTO usage_events(id,workspace_id,user_id,operation,provider,model,input_tokens) VALUES(:id,:w,:u,'chat_question','gemini',:m,:t)"),
        {"id": usage_id, "w": workspace_id, "u": principal.user_id, "m": llm.model, "t": max(1, (len(body.question) + len(context)) // 4)},
    )
    await write_audit(
        db,
        ...
        try:
            ...
            async with SessionLocal() as stream_db:
                await stream_db.execute(
                    text("UPDATE messages SET content=:c,status='completed',completed_at=now() WHERE id=:id"),
                    {"c": answer, "id": assistant_message_id},
                )
                for order, citation in enumerate(citations, 1):
                    await stream_db.execute(
                        text("INSERT INTO message_citations(id,message_id,chunk_id,citation_order) VALUES(:id,:m,:c,:o)"),
                        {"id": str(uuid.uuid4()), "m": assistant_message_id, "c": citation["chunk_id"], "o": order},
                    )
                await stream_db.execute(
                    text("UPDATE usage_events SET output_tokens=:t WHERE id=:id"),
                    {"t": max(1, len(answer) // 4), "id": usage_id},
                )
            ...
            async with SessionLocal() as stream_db:
                await stream_db.execute(
                    text("UPDATE messages SET status='failed',completed_at=now() WHERE id=:id"),
